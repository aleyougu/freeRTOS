...
static BaseType_t prvTestWaitCondition( const EventBits_t uxCurrentEventBits, const EventBits_t uxBitsToWaitFor, const BaseType_t xWaitForAllBits ) ;
	...
	EventGroup_t *pxEventBits;
		if((pxEventGroupBuffer)==0) printf("Error:%s,%d\r\n","FreeRTOS\\event_groups.c",140);
...
BaseType_t xTimeoutOccurred = ( ( BaseType_t ) 0 );
	...
	if((( uxBitsToWaitFor & 0xff000000UL ) == 0)==0) printf("Error:%s,%d\r\n","FreeRTOS\\event_groups.c",334);
	if((uxBitsToWaitFor != 0)==0) printf("Error:%s,%d\r\n","FreeRTOS\\event_groups.c",335);
	{
		if((!( ( xTaskGetSchedulerState() == ( ( BaseType_t ) 0 ) ) && ( xTicksToWait != 0 ) ))==0) printf("Error:%s,%d\r\n","FreeRTOS\\event_groups.c",338);
	}
	vTaskSuspendAll();
	{
...
BaseType_t xMatchFound = ( ( BaseType_t ) 0 );
	if((xEventGroup)==0) printf("Error:%s,%d\r\n","FreeRTOS\\event_groups.c",541);
	if((( uxBitsToSet & 0xff000000UL ) == 0)==0) printf("Error:%s,%d\r\n","FreeRTOS\\event_groups.c",542);
...
const List_t *pxTasksWaitingForBits = &( pxEventBits->xTasksWaitingForBits );
	vTaskSuspendAll();
	{
		...
		{
			if((pxTasksWaitingForBits->xListEnd . pxNext != ( ListItem_t * ) &( pxTasksWaitingForBits->xListEnd ))==0) printf("Error:%s,%d\r\n","FreeRTOS\\event_groups.c",637);
