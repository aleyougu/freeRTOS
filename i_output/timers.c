...
BaseType_t xReturn = ( ( ( BaseType_t ) 0 ) );
	...
	if((xReturn)==0) printf("Error:%s,%d\r\n","FreeRTOS\\timers.c",307);
	return xReturn;
}
	TimerHandle_t xTimerCreate(	const char * const pcTimerName,
								const TickType_t xTimerPeriodInTicks,
	...
	Timer_t *pxNewTimer;
		{
			...
			if((xSize == sizeof( Timer_t ))==0) printf("Error:%s,%d\r\n","FreeRTOS\\timers.c",361);
		}
		if((pxTimerBuffer)==0) printf("Error:%s,%d\r\n","FreeRTOS\\timers.c",366);
...
static void prvInitialiseNewTimer(	const char * const pcTimerName,
									...
									Timer_t *pxNewTimer )  
{
	if((( xTimerPeriodInTicks > 0 ))==0) printf("Error:%s,%d\r\n","FreeRTOS\\timers.c",396);
	if( pxNewTimer != 0 )
	{
		prvCheckForValidListAndQueue();
...
DaemonTaskMessage_t xMessage;
	if((xTimer)==0) printf("Error:%s,%d\r\n","FreeRTOS\\timers.c",422);
	if( xTimerQueue != 0 )
	{
...
TaskHandle_t xTimerGetTimerDaemonTaskHandle( void )
{
	if((( xTimerTaskHandle != 0 ))==0) printf("Error:%s,%d\r\n","FreeRTOS\\timers.c",464);
	return xTimerTaskHandle;
}
TickType_t xTimerGetPeriod( TimerHandle_t xTimer )
{
Timer_t *pxTimer = ( Timer_t * ) xTimer;
	if((xTimer)==0) printf("Error:%s,%d\r\n","FreeRTOS\\timers.c",473);
...
Timer_t * const pxTimer = ( Timer_t * ) ( (&( ( pxCurrentTimerList )->xListEnd ))->pxNext->pvOwner );
	...
	if( pxTimer->uxAutoReload == ( UBaseType_t ) ( ( BaseType_t ) 1 ) )
	{
		if( prvInsertTimerInActiveList( pxTimer, ( xNextExpireTime + pxTimer->xTimerPeriodInTicks ), xTimeNow, xNextExpireTime ) != ( ( BaseType_t ) 0 ) )
		{
			...
			if((xResult)==0) printf("Error:%s,%d\r\n","FreeRTOS\\timers.c",520);
			( void ) xResult;
		}
		else
		{
		...
		if( xMessage.xMessageID >= ( BaseType_t ) 0 )
		{
			...
			switch( xMessage.xMessageID )
			{
				...
				case ( ( BaseType_t ) 9 ) :
					...
					if((( pxTimer->xTimerPeriodInTicks > 0 ))==0) printf("Error:%s,%d\r\n","FreeRTOS\\timers.c",824);
	...
	vPortEnterCritical();
	{
		if( xTimerQueue == 0 )
		{
			...
			pxOverflowTimerList = &xActiveTimerList2;
			{
				...
				static uint8_t ucStaticTimerQueueStorage[ 10 * sizeof( DaemonTaskMessage_t ) ];
				xTimerQueue = xQueueGenericCreateStatic( ( ( UBaseType_t ) 10 ), ( sizeof( DaemonTaskMessage_t ) ), ( &( ucStaticTimerQueueStorage[ 0 ] ) ), ( &xStaticTimerQueue ), ( ( ( uint8_t ) 0U ) ) );
			}
			{
				if( xTimerQueue != 0 )
				{
