...
static void prvAddNewTaskToReadyList( TCB_t *pxNewTCB ) ;
	...
	TaskHandle_t xReturn;
		if((puxStackBuffer != 0)==0) printf("Error:%s,%d\r\n","FreeRTOS\\tasks.c",598);
		if((pxTaskBuffer != 0)==0) printf("Error:%s,%d\r\n","FreeRTOS\\tasks.c",599);
...
BaseType_t xSwitchRequired = ( ( BaseType_t ) 0 );
	...
	if( uxSchedulerSuspended == ( UBaseType_t ) ( ( BaseType_t ) 0 ) )
	{
		...
		{
			{ List_t *pxTemp;
				 if((( ( ( BaseType_t ) ( ( pxDelayedTaskList )->uxNumberOfItems == ( UBaseType_t ) 0 ) ) ))==0) printf("Error:%s,%d\r\n","FreeRTOS\\tasks.c",2522);
...
void vTaskSwitchContext( void )
{
	...
	else
	{
		...
		;
		;
		{ UBaseType_t uxTopPriority; uxTopPriority = ( 31UL - ( uint32_t ) __clz( ( uxTopReadyPriority ) ) ); if((( ( &( pxReadyTasksLists[ uxTopPriority ] ) )->uxNumberOfItems ) > 0)==0) printf("Error:%s,%d\r\n","FreeRTOS\\tasks.c",2807); { List_t * const pxConstList = ( &( pxReadyTasksLists[ uxTopPriority ] ) ); ( pxConstList )->pxIndex = ( pxConstList )->pxIndex->pxNext; if( ( void * ) ( pxConstList )->pxIndex == ( void * ) &( ( pxConstList )->xListEnd ) ) { ( pxConstList )->pxIndex = ( pxConstList )->pxIndex->pxNext; } ( pxCurrentTCB ) = ( pxConstList )->pxIndex->pvOwner; }; };
		;
	}
}
void vTaskPlaceOnEventList( List_t * const pxEventList, const TickType_t xTicksToWait )
{
	if((pxEventList)==0) printf("Error:%s,%d\r\n","FreeRTOS\\tasks.c",2823);
	vListInsert( pxEventList, &( pxCurrentTCB->xEventListItem ) );
	prvAddCurrentTaskToDelayedList( xTicksToWait, ( ( BaseType_t ) 1 ) );
}
void vTaskPlaceOnUnorderedEventList( List_t * pxEventList, const TickType_t xItemValue, const TickType_t xTicksToWait )
{
	if((pxEventList)==0) printf("Error:%s,%d\r\n","FreeRTOS\\tasks.c",2840);
	if((uxSchedulerSuspended != 0)==0) printf("Error:%s,%d\r\n","FreeRTOS\\tasks.c",2844);
...
{
BaseType_t xReturn;
	if((pxTimeOut)==0) printf("Error:%s,%d\r\n","FreeRTOS\\tasks.c",3020);
	if((pxTicksToWait)==0) printf("Error:%s,%d\r\n","FreeRTOS\\tasks.c",3021);
	vPortEnterCritical();
	{
		const TickType_t xConstTickCount = xTickCount;
...
static void prvCheckTasksWaitingTermination( void )
{
	...
	static void prvDeleteTCB( TCB_t *pxTCB )
	{
		( void ) pxTCB;
		{
			...
			{
				if((pxTCB->ucStaticallyAllocated == ( ( uint8_t ) 2 ))==0) printf("Error:%s,%d\r\n","FreeRTOS\\tasks.c",3626)
				;
			}
		}
	}
