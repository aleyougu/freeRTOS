...
Queue_t * const pxQueue = ( Queue_t * ) xQueue;
	...
	{
	Queue_t *pxNewQueue;
		if((uxQueueLength > ( UBaseType_t ) 0)==0) printf("Error:%s,%d\r\n","FreeRTOS\\queue.c",338);
		if((pxStaticQueue != 0)==0) printf("Error:%s,%d\r\n","FreeRTOS\\queue.c",342);
		if((!( ( pucQueueStorage != 0 ) && ( uxItemSize == 0 ) ))==0) printf("Error:%s,%d\r\n","FreeRTOS\\queue.c",346);
		if((!( ( pucQueueStorage == 0 ) && ( uxItemSize != 0 ) ))==0) printf("Error:%s,%d\r\n","FreeRTOS\\queue.c",347);
		{
			volatile size_t xSize = sizeof( StaticQueue_t );
			if((xSize == sizeof( Queue_t ))==0) printf("Error:%s,%d\r\n","FreeRTOS\\queue.c",355);
		}
...
Queue_t * const pxQueue = ( Queue_t * ) xQueue;
	if((pxQueue)==0) printf("Error:%s,%d\r\n","FreeRTOS\\queue.c",927);
	if((!( ( pvItemToQueue == 0 ) && ( pxQueue ->uxItemSize != ( UBaseType_t ) 0U ) ))==0) printf("Error:%s,%d\r\n","FreeRTOS\\queue.c",928);
	if((!( ( xCopyPosition == ( ( BaseType_t ) 2 ) ) && ( pxQueue ->uxLength != 1 ) ))==0) printf("Error:%s,%d\r\n","FreeRTOS\\queue.c",929);
...
UBaseType_t uxSavedInterruptStatus;
Queue_t * const pxQueue = ( Queue_t * ) xQueue;
	if((pxQueue)==0) printf("Error:%s,%d\r\n","FreeRTOS\\queue.c",1084);
	if((pxQueue ->uxItemSize == 0)==0) printf("Error:%s,%d\r\n","FreeRTOS\\queue.c",1088);
	if((!( ( pxQueue ->pcHead == 0 ) && ( pxQueue ->pcTail != 0 ) ))==0) printf("Error:%s,%d\r\n","FreeRTOS\\queue.c",1093);
	vPortValidateInterruptPriority();
	uxSavedInterruptStatus = ulPortRaiseBASEPRI();
	{
...
UBaseType_t uxSavedInterruptStatus;
Queue_t * const pxQueue = ( Queue_t * ) xQueue;
	if((pxQueue)==0) printf("Error:%s,%d\r\n","FreeRTOS\\queue.c",1440);
	if((!( ( pvBuffer == 0 ) && ( pxQueue ->uxItemSize != ( UBaseType_t ) 0U ) ))==0) printf("Error:%s,%d\r\n","FreeRTOS\\queue.c",1441);
	vPortValidateInterruptPriority();
	uxSavedInterruptStatus = ulPortRaiseBASEPRI();
	{
		const UBaseType_t uxMessagesWaiting = pxQueue->uxMessagesWaiting;
...
int8_t *pcOriginalReadPosition;
Queue_t * const pxQueue = ( Queue_t * ) xQueue;
	if((pxQueue)==0) printf("Error:%s,%d\r\n","FreeRTOS\\queue.c",1532);
	if((!( ( pvBuffer == 0 ) && ( pxQueue ->uxItemSize != ( UBaseType_t ) 0U ) ))==0) printf("Error:%s,%d\r\n","FreeRTOS\\queue.c",1533);
	if((pxQueue ->uxItemSize != 0)==0) printf("Error:%s,%d\r\n","FreeRTOS\\queue.c",1534);  
	vPortValidateInterruptPriority();
	uxSavedInterruptStatus = ulPortRaiseBASEPRI();
	{
